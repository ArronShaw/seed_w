...
static void display_banner(parameters_t *params) {
...
static void display_usage_and_exit() {
  printf("%s", usage);
  exit(EXIT_SUCCESS);
}
...
static void param_init(parameters_t *params) {
...
static void process_argv(int argc, char *argv[], parameters_t *params) {
  int i = 1;
...
static void print_motif(char *seed, motif *m, parameters_t *params) {
...
vector_t *words_enumerate(char seed[], vtree_t *vtree[], parameters_t *params) {
  ...
  vector_t *motif_list = dev_new_vector2(
      VECTOR_INI_CAP, INCREMENT); //(initial capacity, increment)
  dstring_t *pattern;

  int cur_support;

  for (int seed_index = 0; seed_index < L - params->word_length + 1;
       seed_index++) {
    char dest[params->word_length + 1];
    cur_support = 1;
...
motif *motif_concatenate(motif *motif_a, motif *motif_b) {
  ...
  if (motif_a != NULL && motif_b != NULL) {
    ...
    while (temp_expression != NULL) {
      ...
      if (new_motif->head == NULL)
        new_motif->head = new_expression;
    ...
    dev_die("motif_a or motif_b is NULL");
  return new_motif;
}
...
int has_enough_support(dstring_t *ds_seed, vtree_t *vtree[], motif *query_motif,
  ...
  for (int i = 1; i < NUM_INPUT_STRING; i++) {
    ...
    if (match_motif(ds_seed, i0, vtree[i], query_motif, params, 0) == 1) {
      cur_support++;
    }
...
void get_neg_support(dstring_t *ds_seed, vtree_t *vtree[], vector_t *motif_list,
                     parameters_t *params) {
...
vector_t *motif_discovery(dstring_t *ds_seed, vector_t *motif_list,
                          vtree_t *vtree[], int control, parameters_t *params) {
  // Discover all motifs that has enough supports

  int one_word_begin = 0; // Begin index of the word length 1
  int one_word_end =
      dev_vector_size(motif_list); // End index of the word length 1
  ...
  while (has_new_motif) {
    ...
    for (int i = con_begin; i < con_end; i++) {
      ...
      for (int j = one_word_begin; j < one_word_end; j++) {
        ...
        // determine if j's index in the seed sequence is greater than i's last
        // word's index
        if (motif_index_com(motif_i, motif_j)) {
          ...
          cur_support = 0;
          if (has_enough_support(ds_seed, vtree, new_motif, &cur_support,
                                 params) == TRUE) {
...
vector_t *motif_ranking(vector_t *motif_list) {
  ...
  for (int i = 0; i < dev_vector_size(motif_list); i++) {
    ...
    // printf("11 = %d, 12 = %d\n", m1->num_match_pos, NUM_INPUT_STRING_POS -
    // m1->num_match_pos);
    // printf("21 = %d, 22 = %d\n", m1->num_match_neg, NUM_INPUT_STRING_NEG -
    // m1->num_match_neg);
    // printf("p_val = %.4e \n", fisher_right_p);
  }
}
...
void save_motifs(char seed[], vector_t *motif_list, parameters_t *params) {
  ...
  for (int i = 0; i < dev_vector_size(motif_list); i++) {
    ...
    fprintf(fh, "  <motif id=\"%d\" offset=\"%d\" pvalue=\"%.4e\">\n", i,
            ((expression *)(m->head))->begin_index, m->p_value);
...
void print_time(clock_t begin, char *s) {
  clock_t end = clock();
  double time_spent = (double)(end - begin) / CLOCKS_PER_SEC;
  printf("Time Spent - %s: %f s\n", s, time_spent);
}
...
int main(int argc, char *argv[]) {
  ...
  /* reading data */
  begin = clock();
  NUM_INPUT_STRING = NUM_INPUT_STRING_POS =
      bio_read_fasta(params.filename_pos, &seqs_pos, &descs_pos, isnuc);
  ...
  print_time(begin, "Read Pos Fas File");
  ...
  print_time(begin, "Word Enumeration");
  ...
  print_time(begin, "Motif Discovery");
  ...
  print_time(begin, "Free Memory");

  begin = clock();
  /* motif discovery - NEG */
  NUM_INPUT_STRING = NUM_INPUT_STRING_NEG =
      bio_read_fasta(params.filename_neg, &seqs_neg, &descs_neg, isnuc);
  ...
  print_time(begin, "Read Neg Fas File");
  ...
  print_time(begin, "get_neg_support");
  ...
  print_time(begin, "motif_ranking");
  ...
  if (params.match_file != NULL) {
    ...
    printf("match file is NULL, motifs will not be saved;\nIf you wish to save "
           "matches, please specify a match file path.\n");
  print_time(begin, "Save motifs");
